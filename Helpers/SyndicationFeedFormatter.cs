...
namespace Demo.Modules.CustomFeeds.Helpers
{
    public class SyndicationFeedFormatter : MediaTypeFormatter
    {
        ...
        {
            if (type == typeof(FeedViewModel) || type == typeof(IList<FeedViewModel>))
                return true;
            else
                return false;
        };
        ...
        private void BuildSyndicationFeed(object models, Stream stream, string contenttype)
        {
            ...
            {
                Title = new TextSyndicationContent("Feeds")
            };

            if (models is IList<FeedViewModel> modelsList)
            {
                foreach (var item in modelsList)
                {
                    items.Add(BuildSyndicationItem(item));
                }
                //var enumerator = modelsEnumeable.GetEnumerator();
            ...
            {
                items.Add(BuildSyndicationItem((FeedViewModel)models));
            }
        ...
        private SyndicationItem BuildSyndicationItem(FeedViewModel u)
        {
            var item = new SyndicationItem()
            {
                Title = new TextSyndicationContent(u.Title),
                BaseUri = new Uri(u.Address),
                LastUpdatedTime = u.CreatedDate,
